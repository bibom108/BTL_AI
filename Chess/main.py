...
def draw_game_state(screen, game_state, valid_moves, square_selected):
    ...
    # Contains: screen, highlight square(if any), pieces
    draw_squares(screen)
...
def draw_pieces(screen, game_state):
    # Display chess piece images into board
    for r in range(DIMENSION):
        for c in range(DIMENSION):
...
def highlight_square(screen, game_state, valid_moves, square_selected):
    if square_selected != () and game_state.is_valid_piece(square_selected[0], square_selected[1]):
        ...
        if (game_state.whose_turn() and game_state.get_piece(row, col).is_player(Player.PLAYER_1)) or \
            ...
            # hightlight selected square - BLUE 
            s = py.Surface((SQ_SIZE, SQ_SIZE))
            s.set_alpha(100)
            s.fill(py.Color("blue"))
            screen.blit(s, (col * SQ_SIZE, row * SQ_SIZE))

            # highlight move squares - GREEN
...
def main():
    ...
    print("You are black!\n")
    while True:         
        human_player = input("Do you want to play first? (y or n)\n")
        if human_player is "y" or human_player is "n":
            break
        else:
            print("Enter y or n.\n")
